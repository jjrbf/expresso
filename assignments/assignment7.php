        ...
        if (isset($_POST['search_txt'])) {
            ...
            if ($CSVfp !== FALSE) {
                ...
                while (! feof($CSVfp)) {
                    ...
                    //search the search string in product id, prod name, prod cost or prod price
                    if (! empty($data)){
                        $prd_id = $data[0];
                        $prd_name=$data[1];
                        $prd_cost=$data[2];
                        ...
                        $prod_price_lower=strtolower(str_replace('$', '', $prd_price));
                        if (str_contains($prod_name_lower, $search_lower) || str_contains($prod_id_lower, $search_lower) || str_contains($prd_cost, $search) || (strpos($prod_price_lower, $search_lower) !== false)) {
